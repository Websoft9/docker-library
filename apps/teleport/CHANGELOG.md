...
- Auto-create the first administrator and a full-access `admin` role on first start from `src/config/bootstrap.yaml` (`--bootstrap`). The admin password is `W9_LOGIN_PASSWORD` in `.env`, stored as a bcrypt hash in the bootstrap file.
...
- Add `tests/cases.yml` with an HTTPS `/webapi/ping` and admin-login check.
