...
本项目是由 [Websoft9](https://www.websoft9.com) 研发的 **云原生程序** ，大大简化 Pimcore  复杂的安装及配置。

## 系统要求

安装本项目，确保符合如下的条件（[详细参考](https://github.com/pimcore/docker#recommended-system-requirements)）：
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r pimcore
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r pimcore -p
```
2.复制package包后安装

将压缩的package包复制到想要安装的服务器或虚拟机，登录 Linux，运行下面的**自动化命令**即可启动安装并显示安装结果。
```
sudo bash install-pimcore
```
...
```
git clone --depth=1 https://github.com/Websoft9/docker-pimcore
cd docker-pimcore
...
| 名称 | 端口号 | 用途 |  必要性 |
| --- | --- | --- | --- |
| pimcore-server | 9002 | 浏览器访问 Pimcore  | Y |
| phpmyadmin | 9090 | 数据库可视化管理工具 | Y |
## 文档

[Pimcore  管理员手册](https://support.websoft9.com/docs/pimcore)

## 企业级支持

如果需要企业级支持，请订阅我们提供的 [Pimcore  企业级支持版](https://apps.websoft9.com/pimcore) 
