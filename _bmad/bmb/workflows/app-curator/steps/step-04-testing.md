...
- **Docker Engine** + **Docker Compose v2** — L1 静态验证（`docker compose config`）需要本地安装
- **SSH 客户端**（`ssh`、`sshpass`） — L2 远程测试需要，本地测试模式不需要
- **Python 3** + `jinja2-cli`、`python-dotenv` — Phase 8 README 生成需要
- 如使用远程测试：`_bmad/bmb/test-servers.yaml` 中需配置可达的服务器
...
```bash
# 从 docker-compose.yml 提取引用的变量（兼容非 GNU grep 环境）
USED_VARS=$(grep -oE '\$\{?[A-Z_][A-Z0-9_]*' docker-compose.yml | sed 's/[${}]//g' | sort -u)
...
# {http_retries} 来自 test-servers.yaml test_settings.http_retries，默认值: 3
...
> ⚠️ **远程执行注意：** 以下所有 curl 和 docker exec 命令均在远程测试服务器上执行。
> 如果使用远程测试模式，所有命令必须通过 `$SSH_CMD` 前缀包装。
> 如果已回退到本地测试模式，则直接在本地执行（无需 `$SSH_CMD`）。
...
#    注意：远程模式下所有命令通过 $SSH_CMD 执行，本地模式直接执行
RESPONSE=$($SSH_CMD "curl --noproxy '*' -sS --max-time 10 http://localhost:{W9_HTTP_PORT_SET}")
...
for asset in $(echo "$RESPONSE" | grep -oP '(href|src)="\K[^"]+\.(css|js)' | head -3); do
  STATUS=$($SSH_CMD "curl --noproxy '*' -sS -o /dev/null -w '%{http_code}' --max-time 5 \"http://localhost:{W9_HTTP_PORT_SET}${asset}\"")
...
for ep in $ENDPOINTS; do
  STATUS=$($SSH_CMD "curl --noproxy '*' -sS -o /dev/null -w '%{http_code}' --max-time 5 \
...
```bash
# 1. 进程检查 — 确认关键进程在运行
$SSH_CMD "docker exec {W9_ID} sh -c 'ps aux 2>/dev/null || ls /proc/*/cmdline 2>/dev/null | \
  while read f; do echo -n \"\$f: \"; cat \"\$f\" 2>/dev/null | tr \"\\0\" \" \"; echo; done' \
  | head -20"

# 2. 端口监听检查（容器内部视角）
$SSH_CMD "docker exec {W9_ID} sh -c 'ss -tlnp 2>/dev/null || netstat -tlnp 2>/dev/null || \
  cat /proc/net/tcp' | head -10"

# 3. 如果应用有 /health 或 /api/health 端点:
$SSH_CMD "docker exec {W9_ID} sh -c \
  'curl -sS --max-time 3 http://localhost:{W9_HTTP_PORT}/health 2>/dev/null || echo \"无 health 端点\"'"

# 4. 对于有数据库的应用 — 验证 DB 连接:
# $SSH_CMD "docker exec {W9_ID} sh -c '{db_check_command}'"
```
...
```bash
# 数据库类应用:
# $SSH_CMD "docker exec {W9_ID} {db_cli} -u {user} -p{password} -e 'SELECT 1' 2>/dev/null"
# 示例: $SSH_CMD "docker exec mysql mysql -uroot -p\$W9_POWER_PASSWORD -e 'SELECT 1'"

# 消息队列类:
# $SSH_CMD "docker exec {W9_ID} {mq_cli} list queues 2>/dev/null"

# CLI 工具类:
# $SSH_CMD "docker exec {W9_ID} {tool} --version"
# $SSH_CMD "docker exec {W9_ID} {tool} status"

# 通用: 检查主进程是否正常运行（退出码 0）
$SSH_CMD "docker exec {W9_ID} sh -c 'kill -0 1 && echo \"✅ PID 1 running\" || echo \"❌ PID 1 not running\"'"
```
...
⚠️ 最大重试次数: 3 次
⏱️ 重试冷却: 每次重试前等待 10 秒（让容器完成清理和端口释放）
```
